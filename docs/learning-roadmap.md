...
当前项目已经达到“学习型 C++ 工程”的合理上限：有业务层、文件仓库、可选 MySQL 仓库、自动化测试、CI、安装和 ZIP 打包。
文件模式是默认模式，MySQL 模式是可选模式；两者通过 `BookRepository` 接口连接，因此业务代码不需要知道数据到底保存在哪里。
不再继续增加 GUI、网络层或复杂设计模式。下一阶段把本项目当作复习材料，转入 `java-learning/`，建立 Java 后端主线。
